...
public class TestActivity extends Activity implements OnTouchListener {
    ...
    public void onButtonClick(View v) {
        if (v == mCreateWindowButton) {
            ...
            //FLAG_NOT_TOUCH_MODAL 当点击控件的时候事件就是控件的,当点击控件外事件就是控件外的,需要开启,否则其他window无法接受到事件
            //FLAG_NOT_FOCUSABLE window不需要焦点和事件直接传递给下层需要焦点的window
            // FLAG_SHOW_WHEN_LOCKED 开启可以显示在屏锁上
